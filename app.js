...
client.voicecmds = new Collection();
...
for (const file of voiceFiles) {
  ...
  client.voicecmds.set(command.data.name, command);
}
...
client.on('interactionCreate', async int => {
  ...
  const command = client.commands.get(int.commandName) ?? client.admincmds.get(int.commandName) ?? client.voicecmds.get(int.commandName);
  ...
  //   cooldownAmount = (timestamps || 1) * 1000;
...
module.exports = { getCommands, getAdminCmds, getEnchants, currency, epicstartdate, client }
